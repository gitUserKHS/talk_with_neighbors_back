...
`APP_PORTFOLIO_DEMO_ENABLED=true`일 때 공개 피드 또는 공개 모임 데이터셋 전체가 비어 있으면,
DB에 사용자를 만들지 않고 코드에 정의된 비식별 예시 콘텐츠를 반환한다. 실제 공개 데이터가 하나라도
생기면 해당 종류의 데모는 즉시 사라지며 실제 데이터와 섞이지 않는다. 응답의 `demo` 필드로 예시
콘텐츠(`true`)와 실제 콘텐츠(`false`)를 구분할 수 있다. 기본값은 `false`이고 AWS k3s 포트폴리오
배포에서만 명시적으로 켠다. `portfolio-demo-*` ID는 영속 엔티티의 UUID 형식과 다르며 작성·참여 등
변경 API의 대상으로 사용하면 안 된다.
