...
`APP_OFFICIAL_CONTENT_ENABLED=true`인 환경에서는 로그인과 매칭이 불가능한 `SYSTEM` 운영팀
계정이 실제 게시글·공식 댓글·공개 모임 엔티티를 멱등하게 준비한다. 글과 일정 occurrence는 결정적
UUID를 사용하므로 재배포해도 중복되지 않고, 회원이 남긴 좋아요·댓글·모임 참가 기록은 보존된다.
공개 응답의 `official=true`로 운영팀 콘텐츠를 투명하게 구분하며 `demo` 응답은 사용하지 않는다.
지난 공식 모임은 다음 동기화 때 종료되고 가까운 향후 일정 두 건이 새 occurrence로 보충되며, 이미
저장된 일정의 시간은 절대 이동하지 않는다.

피드는 작성자가 `publicPreview=true`로 명시적으로 공개한 게시글만 제공한다. 일반 회원 작성자는
`이웃`으로 익명화하고 SYSTEM 콘텐츠만 `이웃톡 운영팀`으로 표시한다. 기존 게시글과 옵션을 생략한
새 게시글은 비공개이며, 댓글 내용과 댓글 작성자 정보는 공개 API에서 제공하지 않는다.
