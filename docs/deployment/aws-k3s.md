...
> **현재 상태 — 2026-07-15:** 기존 HTTP 배포와 E2E 검증은 완료되어 있다. 이 변경은 `talk-with-neighbors.duckdns.org`용 Elastic IP, Traefik HTTP-01 인증서 자동 발급·갱신, HTTP→HTTPS 전환을 코드화했다. 실제 전환은 Terraform plan/apply로 새 Elastic IP를 만든 뒤 DuckDNS A 레코드를 그 IP로 갱신하고 배포 워크플로를 실행해야 완료된다. **Elastic IP를 포함한 AWS 리소스는 비용이 발생할 수 있다.**

이 구성은 고가용성, 무중단 배포, 자동 DB 백업을 보장하지 않는다. 포트폴리오 공개 전에는 아래 TLS 전환 절차와 DB 백업을 실제로 검증해야 한다.
...
1. 프런트 변경을 먼저 `main`에 병합하고 **Publish frontend image**가 성공해 프런트 `:main`이 승격됐는지 확인한다.
2. 그다음 백엔드 변경을 `main`에 병합한다.
3. 백엔드 **Publish backend image**가 성공하면 자동 CD가 새 백엔드 `:main`과 앞 단계의 프런트 `:main` digest를 함께 고정해 배포한다.

이 순서는 새 백엔드가 호환되는 새 프런트보다 먼저 배포되는 것을 막기 위한 릴리스 규칙이다. 두 저장소의 병합을 동시에 진행하지 않는다.

### 8.2 배포 실행

일반 배포는 위 자동 CD를 사용한다. 자동 실행은 `start_if_stopped=true`, `reinitialize_k3s_network=false`, 빈 재초기화 확인 문구로 고정된다. 워크플로는 인스턴스에 Terraform의 Elastic IP가 연결되어 있고 공개 DNS A 레코드가 그 주소와 일치하는지 먼저 확인한다. 조건이 맞아야 Traefik ACME 설정과 TLS Ingress를 적용하고 `Secure` 쿠키를 켠다. AWS 호출 전 `production` Environment 승인 정책은 EC2 시작과 공인 IPv4 비용의 게이트로도 동작한다.
...
1. 자동 실행은 성공한 백엔드 게시 실행과 동일 저장소 `main` 출처를 검증하고 두 `:main` 태그를 OCI digest로 고정한다. 수동 실행은 입력한 digest를 사용한다.
2. 두 경로 모두 Environment 승인, digest 정규식, secret 길이를 검증한다.
...
9. MySQL과 Redis를 먼저 준비하고 버전·체크섬이 검증된 DB migration을 적용한 뒤 백엔드·프론트엔드 rollout과 백엔드 readiness를 확인한다.
10. HTTP 영구 redirect, 유효한 HTTPS `/healthz`, 공개 API와 보호 쓰기 401을 엄격하게 스모크 테스트한다.
11. runner·S3·노드의 평문 임시 파일을 정리한다.
...
DB schema 변경은 이미지 롤백으로 되돌아가지 않는다. Hibernate `ddl-auto=update`는 기존 스키마의 기본 보정에 남아 있지만, 운영 업그레이드에 필요한 변경은 `deploy/k8s/database-migrations/V<version>__<description>.sql`로 버전 관리한다. 배포 노드는 MySQL 준비 뒤 `app_schema_migrations` 원장에 버전·설명·SHA-256을 기록하며, 이미 적용된 버전의 파일이 바뀌면 배포를 거부한다. migration이 모두 성공해야 애플리케이션 rollout을 시작한다.

현재 원장은 기존 Hibernate 스키마에서 점진적으로 명시적 migration 체계로 옮기기 위한 배포 계층이며, 전체 스키마 baseline을 제공하는 Flyway 대체물은 아니다. 새 변경은 하위 호환 가능한 expand/contract 단계로 나누고, 적용된 migration 파일은 수정하지 말고 다음 버전을 추가한다. 파괴적 migration 전에는 논리 백업과 복구 시험을 먼저 한다.
