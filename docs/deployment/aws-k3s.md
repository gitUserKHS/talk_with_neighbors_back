...
2026-07-14에 서울 리전의 myApplications에 `talk-with-neighbors` 애플리케이션을 등록했다. AWS 콘솔 홈의 **애플리케이션** 위젯이나 **myApplications → talk-with-neighbors**에서 대시보드를 열 수 있다. 이 대시보드와 Resource Group은 리소스·비용·상태 인벤토리이며 프런트엔드 화면을 호스팅하거나 렌더링하지 않는다. 실제 웹 UI는 `terraform output -raw application_http_url`의 동적 EC2 주소로 연다. EC2 instance, VPC, subnet, security group, media·deployment·Terraform state S3 bucket 등 핵심 7개 리소스에는 애플리케이션의 `awsApplication` 태그를 적용했다. AWS provider의 `ignore_tags`가 Terraform 관리 리소스에서 이 외부 소유 태그를 보존한다. CloudFormation 관리 state bucket의 애플리케이션 연결은 stack update에서 달라질 수 있으므로 best-effort 표시로 취급한다.
