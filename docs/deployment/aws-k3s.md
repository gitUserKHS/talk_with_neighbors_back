...
두 저장소를 함께 변경하며 단순 API 호환 순서가 필요한 경우에는 새 백엔드와도 호환되는 프런트를 먼저 게시한 뒤 백엔드를 병합한다. 파괴적 API 변경은 한 번에 전환하지 말고 구·신 버전을 함께 지원하는 expand/contract 단계로 나눈다. 단, 이번 모임 달력 전환은 legacy 쓰기를 먼저 흡수해야 하므로 이 일반 순서를 사용하지 않는다. 12.1절의 **A 호환 backend 배포·이전 Pod 제거 확인 → B backfill migration 배포 → C frontend 병합·배포** 순서를 따른다.
...
DB schema 변경은 이미지 롤백으로 되돌아가지 않는다. 운영 Hibernate는 `ddl-auto=none`으로 고정하고, 운영 업그레이드에 필요한 변경은 `deploy/k8s/database-migrations/V<version>__<description>.sql`로 버전 관리한다. 배포 노드는 MySQL 준비 뒤 `app_schema_migrations` 원장에 버전·설명·SHA-256을 기록하며, 이미 적용된 버전의 파일이 바뀌면 배포를 거부한다. migration이 모두 성공해야 애플리케이션 rollout을 시작한다.
...
따라서 **빈 DB/PVC에서 이 migration 원장만 실행하는 신규 설치는 지원하지 않는다.** 새 노드나 PVC를 복구할 때는 먼저 최근 복원 검증을 통과한 schema+data 백업을 복원한 다음 migration을 실행한다. 달력 backfill 직전 preflight가 핵심 테이블·열 fingerprint를 확인하며, baseline이 없거나 불완전하면 데이터를 쓰거나 원장에 성공을 기록하기 전에 `restore a verified schema+data backup first` 오류로 중단한다.

모임 달력 통합 릴리스는 세 단계로 분리한다. **PR A**는 호환 backend Java와 테스트만 배포하고 `V2026071601`을 포함하지 않는다. 이전 프런트의 프로필 일정 쓰기가 canonical schedule에도 원자적으로 동기화되고 모든 이전 Pod가 교체된 것을 먼저 확인한다. **PR B**는 그 최신 `main` 위에 `V2026071601__backfill_chat_schedule_calendar.sql`, runner gate, migration 테스트와 이 운영 문서만 추가해 배포한다. 후보 공개 모임 행은 `FOR UPDATE`로 잠긴 단일 트랜잭션에서 canonical `chat_schedules`로 변환되므로 이미 호환된 backend 쓰기와 snapshot race가 생기지 않는다. 마지막으로 새 프런트를 별도 배포한다. 현재 배포 스크립트는 migration 뒤 새 backend를 rollout하므로 PR A와 PR B를 한 배포로 합치지 않는다.

이 migration은 `chat_rooms.scheduled_at`, `duration_minutes`, `registration_deadline`을 수정하거나 지우지 않는다. 롤백에서도 DB migration은 역적용하지 않으며, 보존된 legacy 열 덕분에 이전 백엔드가 일정을 계속 읽을 수 있다. commit 뒤 원장 기록 전에 실패해도 결정적 ID와 누락 dependent 복구로 재실행할 수 있다.
