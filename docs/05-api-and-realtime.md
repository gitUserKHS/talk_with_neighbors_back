...
| 메서드 | 경로 | 공개 범위 |
|---|---|---|
| GET | `/api/public/feed?page=0&size=20&mode=RECOMMENDED` | 작성자가 `publicPreview=true`로 동의한 게시글만 제공; `RECOMMENDED`, `NEARBY`, `LATEST` 지원, 선택적 `region`은 요청 범위에서만 사용, 작성자는 `이웃`으로 익명화 |
...
| 메서드 | 경로 | 목적 |
|---|---|---|
| GET | `/api/feed?page=0&size=20&mode=RECOMMENDED` | 피드 조회; `RECOMMENDED`, `NEARBY`, `LATEST` 지원, 생략 시 `RECOMMENDED` |
