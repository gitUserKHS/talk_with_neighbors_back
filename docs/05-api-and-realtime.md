...
두 공개 DTO에는 `demo: boolean`이 포함된다. 기본값이 꺼진 `APP_PORTFOLIO_DEMO_ENABLED`를
배포 환경에서 명시적으로 켠 경우에만, 해당 종류의 공개 데이터셋 전체가 0건일 때 비식별 데모
DTO가 반환된다. 실제 공개 데이터와 데모는 섞이지 않는다. 모임 검색은 실제 공개 모임이 하나라도
있으면 검색 결과 0건을 그대로 반환하고, 전체 데이터셋이 비었을 때만 동일한 keyword/interest 및
페이지 조건을 데모 목록에 적용한다. 데모 ID는 표시 전용이며 변경 API에 전달하지 않는다.
